...
use crate::context::{LucidContext, RegisterBank, fault_handler};
use crate::files::FileTable;
use crate::mmu::Mmu;
use crate::err::LucidErr;
use crate::{prompt, fault};
...
fn create_shmem(contextp: *mut LucidContext, base: usize, length: usize)
    -> i32 {
    // Create CString
    ...
    if fd == -1 { fault!(contextp, LucidErr::from("Failed to shm_open")); }
    ...
    if result == -1 { fault!(contextp, LucidErr::from("Failed to ftruncate")); }
    ...
    if bytes_total != length as isize {
        fault!(
            contextp, LucidErr::from("Failed to write snapshot data to shmem"));
    }
    ...
    if result == -1 {
        fault!(
            contextp, LucidErr::from("Failed to unlink shmem object")
        );
    }
...
pub fn take_snapshot(contextp: *mut LucidContext) {
    ...
    prompt!("Snapshot dimensions: 0x{:X} - 0x{:X}",
        context.snapshot.base, context.snapshot.base + context.snapshot.length);
    context.snapshot.fd = create_shmem(
        contextp, context.snapshot.base, context.snapshot.length);
...
pub fn restore_snapshot(contextp: *mut LucidContext) -> Result<(), LucidErr> {
    ...
    if context.dirty_files {
        return Err(
            LucidErr::from("Dirty files detected while restoring snapshot"));
    }
