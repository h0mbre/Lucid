...
# ------------------------------------------------------------
# 9. Build the separate SMP fuzzing Bochs
# ------------------------------------------------------------
RUN cd Bochs/bochs && \
    make distclean || true && \
    CC="/lucid/musl-cross-make/output/bin/x86_64-linux-musl-gcc" \
    ...
    sed -i 's/ -export-dynamic / /' Makefile && \
    make -j$(nproc) && \
    cp bochs /lucid/build/lucid-bochs-smp

# ------------------------------------------------------------
# 10. Install GUI dependencies for snapshot Bochs build
# ------------------------------------------------------------
RUN apt update && apt install -y \
...
# ------------------------------------------------------------
# 11. Build the original single-CPU GUI Bochs for snapshotting
# ------------------------------------------------------------
RUN cd Bochs/bochs && \
    make distclean || true && \
    find . -type f \( -name '*.o' -o -name '*.a' -o -name bochs \) -delete && \
    CC="gcc" \
    CXX="g++" \
...
# ------------------------------------------------------------
# 12. Build the SMP GUI Bochs for snapshotting
# ------------------------------------------------------------
RUN cd Bochs/bochs && \
    make distclean || true && \
    find . -type f \( -name '*.o' -o -name '*.a' -o -name bochs \) -delete && \
    ...
    ./configure \
        ...
        --with-sdl2 && \
    make -j$(nproc) && \
    cp bochs /lucid/build/gui-bochs-smp

# ------------------------------------------------------------
# 13. Install pinned Rust toolchain
# ------------------------------------------------------------
...
# ------------------------------------------------------------
# 14. Copy Lucid source and build it
# ------------------------------------------------------------
COPY . /lucid
...
# ------------------------------------------------------------
# 15. Copy Bochs BIOS files into build directory for ease-of-use
# ------------------------------------------------------------
